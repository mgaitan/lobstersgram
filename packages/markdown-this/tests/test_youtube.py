...
def _patch_oembed(response: object | None = None, *, mocker: MockerFixture) -> unittest.mock._patch:
    return mocker.patch(
...
def test_fetch_youtube_video_extracts_channel_description_and_manual_transcript(*, mocker: MockerFixture) -> None:
    transcript = [SimpleNamespace(text="First sentence"), SimpleNamespace(text="Second sentence")]
    transcript_list = mocker.Mock()
    transcript_list.find_manually_created_transcript.return_value.fetch.return_value = transcript
    _patch_oembed(mocker=mocker)
    mocker.patch("markdown_this.fetchers.fetch_html", return_value=PAGE_HTML)
    api = mocker.patch("markdown_this.fetchers.YouTubeTranscriptApi")
...
def test_fetch_youtube_video_accepts_supported_url_shapes(*, mocker: MockerFixture) -> None:
    urls = [
        VIDEO_URL,
    ...
    mocker.patch("markdown_this.fetchers._fetch_youtube_transcript", return_value=None)
    for url in urls:
...
def test_fetch_youtube_video_returns_none_when_oembed_fails(*, mocker: MockerFixture) -> None:
    mocker.patch(
        "markdown_this.fetchers.requests.get",
...
def test_fetch_youtube_video_handles_empty_optional_content(*, mocker: MockerFixture) -> None:
    response = mocker.Mock()
    response.raise_for_status.return_value = None
    response.json.return_value = {"title": "", "author_name": ""}
    _patch_oembed(response, mocker=mocker)
    mocker.patch("markdown_this.fetchers.fetch_html", return_value="")
    mocker.patch("markdown_this.fetchers._fetch_youtube_transcript", return_value=None)
    assert fetch_youtube_video(VIDEO_URL) is None


def test_youtube_description_extracts_meta_tag_and_handles_request_errors(*, mocker: MockerFixture) -> None:
    mocker.patch("markdown_this.fetchers.fetch_html", return_value=PAGE_HTML)
    ...
    mocker.patch("markdown_this.fetchers.fetch_html", return_value='<meta name="other" content="x">')
    assert fetchers_module._fetch_youtube_description(VIDEO_ID) == ""


def test_youtube_transcript_prefers_manual_transcript(*, mocker: MockerFixture) -> None:
    transcript_list = mocker.Mock()
...
def test_youtube_transcript_falls_back_to_generated_transcript(*, mocker: MockerFixture) -> None:
    transcript_list = mocker.Mock()
...
def test_youtube_transcript_handles_missing_and_failed_transcripts(*, mocker: MockerFixture) -> None:
    api = mocker.patch("markdown_this.fetchers.YouTubeTranscriptApi")
...
def test_extract_main_content_uses_youtube_special_handler(*, mocker: MockerFixture) -> None:
    special_extractor = mocker.Mock(return_value=("Video title", "**Description:** Video body"))
    mocker.patch.object(extractor_module, "SPECIAL_URL_EXTRACTORS", (special_extractor,))
